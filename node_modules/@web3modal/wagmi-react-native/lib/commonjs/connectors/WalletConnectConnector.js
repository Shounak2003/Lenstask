...
class WalletConnectConnector extends _wagmi.Connector {
  ...
  async connect() {
    ...
    try {
      ...
      if (!targetChainId) {
        const store = await _storageUtils.StorageUtils.getItem(STORE_KEY);
      ...
      if (!provider.session || isChainsStale) {
        ...
        this._setRequestedChainsIds(this.chains.map(_ref => {
          let {
            id
          } = _ref;
          return id;
  ...
  _setRequestedChainsIds(chains) {
    _storageUtils.StorageUtils.setItem(REQUESTED_CHAINS_KEY, chains);
  }
  async _getRequestedChainsIds() {
    const requestedChains = await _storageUtils.StorageUtils.getItem(REQUESTED_CHAINS_KEY);
